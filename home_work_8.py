...
def import_data(file_to_add, phonebook):
    try:
        with open(file_to_add, 'r', encoding='utf-8') as new_contacts, open(phonebook, 'a', encoding='utf-8') as file:
            ...
            print(f"В файле {file_to_add} найдено {len(contacts_to_add)} строк.")
            line_number = int(input("Введите номер строки для копирования: "))
            if line_number <= 0 or line_number > len(contacts_to_add):
                print("Неправильно указан номер строки.")
                return
            line_to_copy = contacts_to_add[line_number - 1]
            file.write(line_to_copy)
            print("Данные успешно скопированы.")
    except FileNotFoundError:
        print(f'{file_to_add} не найден. Повторите попытку.')
